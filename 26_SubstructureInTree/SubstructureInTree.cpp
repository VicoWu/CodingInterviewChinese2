...
bool HasSubtree(BinaryTreeNode* pRoot1, BinaryTreeNode* pRoot2)
{
    ...
    {
        if(Equal(pRoot1->m_dbValue, pRoot2->m_dbValue)) //根节点相同， 开始判断是否p2是p1的子树
            result = DoesTree1HaveTree2(pRoot1, pRoot2);
        if(!result) //根节点不同，开始递归往p1的子树上查找
...
// 这个方法是已经知道pRoot1和pRoot2的值相同，然后递归确认子节点是否相同
bool DoesTree1HaveTree2(BinaryTreeNode* pRoot1, BinaryTreeNode* pRoot2)
{
    if(pRoot2 == nullptr) //右侧树已经到达空节点了，返回true
        return true;

    if(pRoot1 == nullptr) // 右侧子树不为空，但是左侧已经为空了，说明不是子树
        return false;

    if(!Equal(pRoot1->m_dbValue, pRoot2->m_dbValue))
        return false;

    return DoesTree1HaveTree2(pRoot1->m_pLeft, pRoot2->m_pLeft) && // 左侧完全相同并且右侧完全相同
