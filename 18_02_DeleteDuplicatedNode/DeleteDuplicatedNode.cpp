...
比较:
参考leetcode 83: Remove dumplicate node for sorted list(retain at least one node ) https://leetcode.com/problems/remove-duplicates-from-sorted-list/
参考leetcode 80: Remove Duplicates from Sorted Array II
...
void DeleteDuplication(ListNode** pHead)
{
    ...
    while(pNode != nullptr)
    {
        ...
        if(!needDelete) //如果pNode指向的节点和下一个节点的值不一样，那么pNode不需要删除
        {
            pPreNode = pNode; // PpreNode向下移动一个位置
            pNode = pNode->m_pNext;  // pNode也向下移动一个位置
        }
        else //相等，那么， pNode和pNode->next肯定都需要删除
        {
            int value = pNode->m_nValue;
            ListNode* pToBeDel = pNode;
            while(pToBeDel != nullptr && pToBeDel->m_nValue == value) //pTioDel一直移动到最后一个相同的节点
            {
            ...
            if(pPreNode == nullptr) //pPreNode是null，说明pPreNode指向的也是相同节点，需要删除
                *pHead = pNext;
            else
                pPreNode->m_pNext = pNext;  //pPreNode指向
            pNode = pNext;
        }
    }
}
